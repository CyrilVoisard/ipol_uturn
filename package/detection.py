...
def quality_evaluation(uturns, uturn_val, n):
    ...
    amp_ref = 3
    q_tot[2] = round(100 * (1 - abs(amp_ref - np.mean(abs(uturn_val))) / amp_ref))
    print("test", q_tot, np.mean(abs(uturn_val)), abs(amp_ref - np.mean(uturn_val)) / amp_ref, abs(amp_ref - np.mean(abs(uturn_val))) / amp_ref)
    print(test)
